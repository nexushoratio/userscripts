...
/**
 * This namespace contains all of the
 * [NexusHoratio](https://github.com/nexushoratio/userscripts/blob/main/lib/README.md)
 * libraries, each in a nested namespace.
 * @namespace NexusHoratio
 */
window.NexusHoratio ??= {};
 ...
 *
 * All {@link NexusHoratio} libraries should throw an `Error` object with the
 * {@link
 * https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause
 * cause} property populated.
 *
 * @external Error
...
/**
 * The JavaScript `JSON` namespace object.
 * @external JSON
 ...
 * API](https://developer.mozilla.org/en-US/docs/Web/API) aware, except
 * `Logger`'s use of `console` and `uuId`'s use of `crypto`.
 *
 * Depends on:
 * - {@link NexusHoratio.xunit}
 * @version 74
 * @license [GPL-3.0-or-later]{@link https://www.gnu.org/licenses/gpl-3.0-standalone.html}
 * @namespace NexusHoratio.base
 */
window.NexusHoratio.base = (function base() {
  'use strict';

  /**
   * @const {number} - Bumped per per release.
   * @memberof NexusHoratio.base
   * @default
   */
  const version = 74;

  /**
   * @const {number} - Returned by some APIs like `findIndex()`.
   * @memberof NexusHoratio.base
   * @default -1
   */
  const NOT_FOUND = -1;
  ...
  /**
   * @const {number} - Useful for testing length of an array.
   * @memberof NexusHoratio.base
   * @default
   */
  const ONE_ITEM = 1;
  ...
  /**
   * @const {number} - Identify the last item in a collection like `at(-1)`.
   * @memberof NexusHoratio.base
   * @default -1
   */
  const LAST_ITEM = -1;
  ...
  /**
   * Mapping of Google's [Canonical Error Codes]{@link
  ...
  /**
   * @typedef {object} NexusHoratio.base~Version
   ...
   *
   * @memberof NexusHoratio.base
   * @param {NexusHoratio.base~Version[]} versions - Versions required.
   * @returns {object} Namespace with only ensured libraries present.
   * @throws {external:Error} When requirements not met.
   */
  function ensure(versions) {
    ...
    for (const ver of versions) {
      ...
      const lib = window.NexusHoratio[libraryName];
      if (!lib) {
        msg = `Library "${libraryName}" is not loaded`;
        opts = {
          cause: {
      ...
      if (minVersion > lib.version) {
        msg = 'Minimum version of library is not loaded';
        opts = {
          cause: {
            code: Code.FAILED_PRECONDITION,
            reason: 'NotMinimumVersion',
            library: libraryName,
            minVersion: minVersion,
            loadedVersion: lib.version,
          },
        };
        throw new Error(msg, opts);
      }
      namespace[libraryName] = lib;
    }
    return namespace;
  }

  const NH = ensure([{name: 'xunit', minVersion: 61}]);
    ...
    testMinVersion() {
      this.assertRaisesCause(
        Error,
        {
          ...
          reason: 'NotMinimumVersion',
          library: 'base',
          minVersion: version + ONE_ITEM,
          loadedVersion: version,
        },
        () => {
          ensure([{name: 'base', minVersion: version + ONE_ITEM}]);
        }
      );
    }
   ...
   *
   * It takes a fixed list of event types upon construction.  Attempts to use
   * an unknown event will throw an error.
   *
   * One approach is to define both present and past tense events so that
   * handlers can prepare for and react to events.
   *
   * @example
   *  const events = [
  ...
  class Dispatcher {
    ...
    /** @typedef {string} NexusHoratio.base.Dispatcher~EventType */

    /**
     * @callback NexusHoratio.base.Dispatcher~Handler
     * @param {NexusHoratio.base.Dispatcher~EventType} eventType - Event type.
     * @param {object} data - Event data.
     */

    /**
     * @param {...NexusHoratio.base.Dispatcher~EventType} eventTypes - Event
     * types this instance can handle.
     */
     ...
     * function to call.
     * @returns {NexusHoratio.base.Dispatcher} This instance, for chaining.
     */
    on(eventType, func) {
    ...
    off(eventType, func) {
      ...
      while ((index = handlers.indexOf(func)) !== NOT_FOUND) {
        handlers.splice(index, ONE_ITEM);
      }
      return this;
    }
     ...
     * @param {NexusHoratio.base.Dispatcher~EventType} eventType - Event type
     * to look up.
     * @throws {external:Error} When eventType was not registered during
     * instantiation.
     * @returns {NexusHoratio.base.Dispatcher~Handler[]} Handlers currently
     * registered for this eventType.
     */
  ...
  class DispatcherTestCase extends NH.xunit.TestCase {
    ...
    testEnabled() {
      // Assemble
      const dispatcher = new Dispatcher('in-the-hole');
      const calls1 = [];
      const calls2 = [];
      const handler1 = (...args) => {
        calls1.push(args);
      ...
      const handler2 = (type, data) => {
        calls2.push([type, data]);
      };
      dispatcher.on('in-the-hole', handler1)
        .on('in-the-hole', handler2);

      // Assert
      this.assertTrue(dispatcher.enabled, 'defaults enabled');
      this.assertEqual(calls1, [], 'calls1 empty');
      this.assertEqual(calls2, [], 'calls2 empty');

      // Act I - Default
      dispatcher.fire('in-the-hole', 'lorem ipsum');

      // Assert
      this.assertEqual(
        calls1, [['in-the-hole', 'lorem ipsum']], 'handler1 called'
      );
      this.assertEqual(
        calls2, [['in-the-hole', 'lorem ipsum']], 'handler2 called'
      );
      ...
      calls1.length = 0;
      calls2.length = 0;
      dispatcher.fire('in-the-hole', 'dolor sit');

      // Assert
      this.assertFalse(dispatcher.enabled, 'toggled to disabled');
      this.assertEqual(calls1, [], 'calls1 empty after disabled');
      this.assertEqual(calls2, [], 'calls2 empty after disabled');

      // Act II - Reenable
      dispatcher.enabled = true;
      dispatcher.fire('in-the-hole', 'amet consectetur');

      // Assert
      this.assertTrue(dispatcher.enabled, 'toggled to enabled');
      this.assertEqual(
        calls1, [['in-the-hole', 'amet consectetur']], 'handler1 reenabled'
      );
      this.assertEqual(
        calls2, [['in-the-hole', 'amet consectetur']], 'handler2 reenabled'
      );
    }
   ...
   *
   * This is similar to the WEB API's {@link
  ...
  class MessageQueue {
    ...
    /**
     * @param {...object} items - Whatever to add to the queue.
     * @returns {NexusHoratio.base.MessageQueue} This instance, for chaining.
     */
    post(...items) {
    ...
    /**
     * @param {function(...object)} [func] - Function that receives the
     * messages.  If falsy, the listener is removed.
     * @returns {NexusHoratio.base.MessageQueue} This instance, for chaining.
     */
    listen(func) {
      if (func) {
  ...
  /**
   * {@link NexusHoratio} libraries and apps should post issues to this {@link
   * NexusHoratio.base.MessageQueue}.
   *
   ...
   *
   * The existence of the {@link NexusHoratio.base~NumberOp#toJSON toJSON},
   * {@link NexusHoratio.base~NumberOp#toString toString}, and {@link
   * NexusHoratio.base~NumberOp#valueOf valueOf}, methods will probably allow
   * this class to work in many situations through type coercion.
   * @memberof NexusHoratio.base~
   */
  class NumberOp {

    /**
     * @param {number} value - Initial value, parsed by {@link external:Number
     * Number}.
     */
    constructor(value) {
      this.assign(value);
     ...
     * @returns {NexusHoratio.base~NumberOp} This instance.
     */
    assign(value = 0) {
  ...
  /**
   * Factory function is not the right type.
   * @memberof NexusHoratio.base~
   */
  class FactoryTypeError extends TypeError {}

  /**
   * Subclass of {@link external:Map Map} that is similar to Python's
   * defaultdict.
   *
   ...
   *
   * The factory function may take arguments.  When {@link
   * NexusHoratio.base.DefaultMap#getWithArguments getWithArguments} is used,
   * those arguments will be passed to the factory.
   * @memberof NexusHoratio.base
   * @extends external:Map
   */
  class DefaultMap extends Map {

    /**
     * @param {function(...args) : object} factory - Function that creates a
     * new default value if a requested key is not present.
     * @param {Iterable} [iterable] - Passed to {@link external:Map}'s super.
     */
    constructor(factory, iterable) {
      if (!(factory instanceof Function)) {
        const msg = `Factory must be a Function, not ${typeof factory}.`;
        throw new FactoryTypeError(msg);
      }
      super(iterable);
    ...
    /**
     * @param {object} key - The key of the element to return from this
     * instance.
     * @returns {object} The value associated with the key, perhaps newly
     * created.
     */
    get(key) {
      if (!this.has(key)) {
    ...
    /**
     * Variant of {@link external:Map#get} that passes args to the factory.
     *
     * @param {object} key - The key of the element to return from this
     * instance.
     * @param {...object} args - Extra arguments passed to the factory
  ...
  class DefaultMapTestCase extends NH.xunit.TestCase {
    ...
    testFactoryWithArgs() {
      ...
      // Act
      dummy.get('a');
      dummy.getWithArguments('c', 7);
      dummy.getWithArguments('d');
   ...
   * intro/outro method pairs, {@link NexusHoratio.base.Logger#entered
   * entered}/{@link NexusHoratio.base.Logger#leaving leaving} and {@link
   * NexusHoratio.base.Logger#starting starting}/{@link
   * NexusHoratio.base.Logger#finished finished}.  By default, the former are
   * opened and the latter collapsed (documented here as `closed`).
   *
   * Individual Loggers can be enabled/disabled by setting the {@link
   * NexusHoratio.base.Logger~Config#enabled enabled} boolean property.
   *
   * Each Logger will have also have a collection of {@link
   * NexusHoratio.base.Logger~Group Group}s associated with it.  These groups
   * can have one of three modes: `opened`, `closed`, `silenced`.  The first
   * two correspond to the browser console nested message groups.  The intro
   * and outro type of methods will handle the nesting.  If a group is set as
   * `silenced`, no messages will be sent to the console.
   *
   ...
   * {@link NexusHoratio.base.Logger.configs configs} property.  The object
   * could be saved via the userscript manager.  Depending on which manager,
   * it may have to be processed with {@link external:JSON JSON} functions.
   * Once exported, the object may be modified.  This could be used to provide
   * a UI to edit the object, though no schema is provided.
   *
   * Some values may be of interest to users for help in debugging a script.
   *
   * The {@link NexusHoratio.base.Logger~Config#callCount callCount} value is
   * how many times a logger would have been used for messages, even if the
   * logger is disabled.  Similarly, each group associated with a logger also
   * has a {@link NexusHoratio.base.Logger~Group#callCount callCount}.  These
   * values can be used to determine which loggers and groups generate a lot
   * of messages and could be disabled or silenced.
   *
   * The {@link NexusHoratio.base.Logger~Config#sequence sequence} value is a
   * rough indicator of how recently a logger or group was actually used.  It
   * is purposely not a timestamp, but rather, more closely associated with
   * how often configurations are restored, e.g. during web page reloads.  A
   * low sequence number, relative to the others, may indicate a logger was
   * renamed, groups removed, or simply parts of an application that have not
   * been visited recently.  Depending on the situation, one could clean up
   * old configs, or explore other parts of the script.
   *
   * Handlers may be attached to listen for changes to the configuration
   * singleton using the {@link NexusHoratio.base.Logger.onConfig onConfig}
   * and {@link NexusHoratio.base.Logger.offConfig offConfig} static methods.
   * Arrival of messages may controlled using {@link
   * NexusHoratio.base.Logger.configDispatcherEnabled configDispatcherEnabled}
   * (useful for disabling during testing).
   *
   * @example
  ...
  class Logger {
    ...
    /**
     * @typedef {object} NexusHoratio.base.Logger~ConfigMutationRecord
     * @property {string} logger - Name of the affected Logger instance.
     * @property {string?} group - Name of a possibly affected group.
     */

    /**
     * @callback NexusHoratio.base.Logger~ConfigMutationHandler
     * @param {string} evt - Type of mutation.
     * @param {NexusHoratio.base.Logger~ConfigMutationRecord} record - Details
     * about the mutation.
     */

    /** @param {string} loggerName - Name for this logger. */
    constructor(loggerName) {
      this.#mq.listen(this.#errMsgListener);
      this.#name = loggerName;
      this.#config = Logger.config(this.#name);
    ...
    /** @type {boolean} */
    static get configDispatcherEnabled() {
      return this.#configDispatcher.enabled;
    }

    // eslint-disable-next-line require-jsdoc
    static set configDispatcherEnabled(val) {
      this.#configDispatcher.enabled = Boolean(val);
    }
    ...
    static set configs(val) {
      const enabled = Logger.configDispatcherEnabled;
      Logger.configDispatcherEnabled = false;

      try {
        Logger.#fromPojo(val);
        Logger.#resetLoggerConfigs();
      } finally {
        Logger.configDispatcherEnabled = enabled;
      }
    }
     ...
     *
     * @param {NexusHoratio.base.Logger~ConfigMutationHandler} callback -
     * Function that receives events.
     */
    static onConfig(callback) {
      for (const evt of this.#configEvents) {
        this.#configDispatcher.on(evt, callback);
      }
    }
    ...
    static offConfig(callback) {
      for (const evt of this.#configEvents) {
        this.#configDispatcher.off(evt, callback);
      }
    }
     ...
     *
     * @param {string} loggerName - Logger configuration to get.
     * @returns {NexusHoratio.base.Logger~Config} Current config for that
     * Logger.
     */
    static config(loggerName) {
      return this.#configs.getWithArguments(loggerName, loggerName);
    }
    ...
    /**
     * @type {boolean}
     * @alias NexusHoratio.base.Logger~Config#enabled
     */
    get enabled() {
    ...
    /** @type {NexusHoratio.base.MessageQueue} */
    get mq() {
      return this.#mq;
    }
    ...
    get silenced() {
      let ret = false;
      const group = this.#groupStack.at(LAST_ITEM);
      if (group) {
     ...
     * @param {...object} rest - Arbitrary items to pass to console.debug.
     */
    log(msg, ...rest) {
      this.#log(msg, ...rest);
    }
    ...
    static #Config = class {
      ...
      /**
       * @alias NexusHoratio.base.Logger~Config
       * @param {string} loggerName - Name of new logger config.
       */
      constructor(loggerName) {
        this.#loggerName = loggerName;
        this.#fireConfigChange('logger');
      }

      /**
       * @type {number}
       * @alias NexusHoratio.base.Logger~Config#sequence
       */
      sequence = 0;

      /**
       * @type {NexusHoratio.base~NumberOp}
       * @alias NexusHoratio.base.Logger~Config#callCount
       */
      get callCount() {
      ...
      set enabled(val) {
        this.#enabled = Boolean(val);
        this.#fireConfigChange('enabled');
      }

      /**
       * Per group settings.
       * @type {Map<string,NexusHoratio.base.Logger~Group>}
       */
      get groups() {
        return this.#groups;
      }
      ...
      set includeStackTrace(val) {
        ...
        this.#fireConfigChange('includeStackTrace');
      }

      /**
       * @param {string} groupName - Name of the group to get.
       * @param {NexusHoratio.base.Logger~GroupMode} mode - Default mode if
       * not seen before.
       * @returns {NexusHoratio.base.Logger~Group} Requested group, perhaps
       * newly made.
       */
      group(groupName, mode) {
        const defaultMode = mode ?? 'opened';
        return this.#groups.getWithArguments(
          groupName, defaultMode, this.#loggerName, groupName
        );
      }

      /**
       * Capture fact that the associated Logger was used.
       *
       * @param {string} groupName - Which group was used.
       */
      used(groupName) {
        const grp = this.group(groupName);

        this.callCount.add(ONE_ITEM);
        this.sequence = Logger.sequence;

        grp.callCount.add(ONE_ITEM);
        grp.sequence = Logger.sequence;

        this.#fireConfigChange('used', groupName);
      }
      ...
      /**
       * @param {string} evt - Config change event to fire.
       * @param {string?} group - Name of group for {@link
       * NexusHoratio.base.Logger~ConfigMutationRecord ConfigMutationRecord}.
       */
      #fireConfigChange = (evt, group) => {
        Logger.#configDispatcher.fire(evt, {
    ...
    static #Group = class {
      ...
      /**
       * @alias NexusHoratio.base.Logger~Group
       * @param {NexusHoratio.base.Logger~GroupMode} mode - Initial mode for
       * this group.
       * @param {string} logger - Name of associated {@link
       * NexusHoratio.base.Logger Logger} instance.
       * @param {string} group - Name of this group.
       */
      constructor(mode, logger, group) {
        this.mode = mode;
        this.sequence = 0;
        this.#logger = logger;
        this.#group = group;
        this.#fireConfigChange('group');
      }

      /**
       * @type {NexusHoratio.base~NumberOp}
       * @alias NexusHoratio.base.Logger~Group#callCount
       */
      get callCount() {
        return this.#callCount;
      }

      /** @type {NexusHoratio.base.Logger~GroupMode} */
      get mode() {
        return this.#mode;
      }
    ...
    static #GroupMode = class {
       ...
       *
       * @alias NexusHoratio.base.Logger~GroupMode
       ...
       */
      constructor(modeName, greeting, farewell, func) {  // eslint-disable-line max-params
        this.#farewell = farewell;
        this.#func = func;
        this.#greeting = greeting;
        this.#name = modeName;

        Logger.#GroupMode.#known.set(this.#name, this);
       ...
       *
       * @param {string} modeName - Mode name.
       * @returns {NexusHoratio.base.Logger~GroupMode} Mode, if found.
       */
      static byName(modeName) {
        return this.#known.get(modeName);
      }
      ...
      /**
       * The console.func to use for opening group.
       * @type {string}
       */
      get func() {
        return this.#func;
    ...
    static {
      Object.freeze(Logger.#configEvents);
      this.#configDispatcher = new Dispatcher(...Logger.#configEvents);
    }

    static #configs = new DefaultMap((...x) => new Logger.#Config(...x));
    ...
    static #fromPojo = (pojo) => {
      if (pojo && pojo.type === 'LoggerConfigs') {
        ...
        for (const [k, v] of Object.entries(pojo.entries)) {
          this.#configs.getWithArguments(k, k)
            .fromPojo(v);
        }
        Logger.sequence += 1;
      }
    ...
    /**
     * This only resets Logger instances that have known configs.
     *
     ...
     * @param {NexusHoratio.base.Logger~GroupMode} defaultMode - Mode to use
     * if new.
     * @param {...object} rest - Arbitrary items to pass to console.debug.
     */
  ...
  class LoggerTestCase extends NH.xunit.TestCase {
    ...
    testCountsCollected() {
      ...
      // Some of these are {@link NexusHoratio.base~NumberOp}
      this.defaultEqual = this.equalValueOf;
    ...
    testTooManyOutros() {
      ...
      // Assert
      this.assertEqual(messages, [], 'should always be a group named "null"');
    }
   ...
   *
   * @param {NexusHoratio.base.Logger} logger - Logger to use.
   ...
   * Basic test runner.
   *
   * This depends on {@link NexusHoratio.base.Logger Logger}, hence the
   * location in this file instead of {@link NexusHoratio.xunit}.
   * @memberof NexusHoratio.base~
   */
  function runTests() {
   ...
   * hash](https://en.wikipedia.org/wiki/Jenkins_hash_function#one_at_a_time).
   *
   * @memberof NexusHoratio.base
  ...
  /* eslint-disable require-jsdoc */
  class StrHashTestCase extends NH.xunit.TestCase {

    testBasic() {
      const tests = [
        {text: '', expected: '00000000'},
        {text: 'test', expected: '3f75ccc1'},
        {text: 'abc123', expected: '599f3312'},
        {text: 'abc124', expected: '238046d5'},
        {text: 'áÈîȮůȲ', expected: '3edc6a0d'},
        {text: 'The quick red fox jumped over the lazy brown dog.',
          expected: 'f6c27f4f'},
      ];
      for (const {text, expected} of tests) {
        this.assertEqual(strHash(text), expected, text);
      }
    }
  ...
  function simpleParseWords(text) {
    ...
    while (working.length || moreWork.length) {
      ...
      if (word) {
        ...
        if (split.length > ONE_ITEM || moreWork.length) {
          moreWork.push(...split);
        } else {
          results.push(word);
        }
   ...
   *
   * Subclasses should NOT override methods here, except for the
   * `constructor`.  Instead they should register listeners for appropriate
   * events.
   *
   * @example
  ...
  class Service {
    ...
    /** @param {string} shortName - Custom name portion of this instance. */
     ...
     *
     * @fires NexusHoratio.base.Service#activate
     * @fires NexusHoratio.base.Service#activated
     */
    activate() {
     ...
     *
     * @fires NexusHoratio.base.Service#deactivate
     * @fires NexusHoratio.base.Service#deactivated
     */
    deactivate() {
     ...
     * @param {NexusHoratio.base.Dispatcher~Handler} func - Single argument
     * function to call.
     * @returns {NexusHoratio.base.Service} This instance, for chaining.
     */
    on(eventType, func) {
    ...
    setShortName(shortName) {
      this.#name = `${this.constructor.name}: ${shortName}`;
     ...
     * NexusHoratio.base.Service~eventTypes - Events used by this class.
     */
    static #eventTypes = [

      /** @event NexusHoratio.base.Service#activate */
      'activate',

      /** @event NexusHoratio.base.Service#activated */
      'activated',

      /** @event NexusHoratio.base.Service#deactivate */
      'deactivate',

      /** @event NexusHoratio.base.Service#deactivated */
      'deactivated',
    ];

    #activated = false
    #allowReactivation = true
    #dispatcher = new Dispatcher(...Service.#eventTypes)
    #logger
    #name
    #shortName
  ...
  class ServiceTestCase extends NH.xunit.TestCase {
    ...
    testProperties() {
      ...
      this.assertEqual(
        s.logger.name,
        'Test: The ServiceTestCase.testProperties',
        'logger name'
      );

      // Act/Assert
      this.assertEqual(s.setShortName('new name'), s, 'chains');

      // Assert
      this.assertEqual(
        s.name, 'Test: new name', 'new long name'
      );
      this.assertEqual(
        s.shortName, 'new name', 'new short name'
      );
      this.assertEqual(
        s.logger.name,
        'Test: new name',
