...
window.NexusHoratio.base = (function base() {
  ...
  /** @type {number} - Returned by some APIs like `findIndex()`. */
  const NOT_FOUND = -1;

  /** @type {number} - Useful for testing length of an array. */
  const ONE_ITEM = 1;

  /**
   * @type {number} - Identify the last item in a collection like `at(-1)`.
   */
  const LAST_ITEM = -1;
   ...
   *
   * @param {Version[]} versions - Versions required.
  ...
  class Dispatcher {
    ...
    /** @type {boolean} - Whether dispatching is currently enabled. */
    get enabled() {
      return this.#enabled;
    }

    /** @param {boolean} val - Set if dispatching is currently enabled. */
    set enabled(val) {
   ...
   *
   * The factory function may take arguments.  When `.getWithArguments()` is
   * used, those arguments will be passed to the factory.
   */
  class DefaultMap extends Map {
     ...
     *
     * @param {*} key - The key of the element to return from this instance.
     * @param {...*} args - Extra arguments passed to the factory function if
     * it is called.
     * @returns {*} - The value associated with the key, perhaps newly
     * created.
     */
   ...
   * userscript manager.  Depending on which one, it may have to be processed
   * with the JSON.{stringify,parse} functions.  Once exported, the object may
   * be modified.  This could be used to provide a UI to edit the object,
   ...
   * on the situation, one could clean up old configs, or explore other parts
   * of the script.
   *
   * Handlers may be attached to listen for changes to the configuration
   * singleton using the {@link Logger.onConfig} and {@link offConfig} static
   * methods.  Arrival of messages may controlled using {@link
   * Logger.configDispatcherEnabled} (useful for disabling during testing).
   *
   * @example
  ...
  class Logger {
     ...
     *
     * @param {ConfigMutationHandler} callback - Function that receives
     * events.
     */
    ...
    static #Group = class {
       ...
       * @param {string} logger - Name of associated {@link Logger} instance.
