...
window.NexusHoratio.base = (function base() {
  ...
  function testLogger() {
    ...
    tests.set('openedGroup', {test: () => {
      ...
      logger.entered('ent');
      return Logger.config('openedGroup').groups.get('ent').name;
    },
    expected: 'opened'});
    ...
    tests.set('closedGroup', {test: () => {
      ...
      return Logger.config('closedGroup').groups.get('start').name;
    },
    expected: 'closed'});
    ...
    tests.set('restoreConfigs', {test: () => {
      const results = [];
      Logger.config('restoreConfigs').includeStackTrace = true;
      results.push(Logger.config('restoreConfigs').includeStackTrace);
