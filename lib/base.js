...
window.NexusHoratio.base = (function base() {
  ...
  const NH = ensure([{name: 'xunit', minVersion: 39}]);
  ...
  class DispatcherTestCase extends NH.xunit.TestCase {
    ...
    testFire() {
      ...
      this.assertEqual(calls1.length, 0, 'calls1 empty');
      this.assertEqual(calls2.size, 0, 'calls2 empty');

      dispatcher.on('boo', handler1);
      dispatcher.fire('boo', 'more random data');
      this.assertEqual(
        calls1, [['boo', 'more random data']], 'single handler1 registered'
      );
      this.assertEqual(calls2.size, 0, 'calls2 still empty');

      calls1.length = 0;
      calls2.clear();
      dispatcher.on('boo', handler1);
      dispatcher.on('boo', handler2);
      dispatcher.fire('boo', {an: 'object'});
      this.assertEqual(
        calls1,
      ...
      this.assertEqual(
        calls2, new Map([['boo', {an: 'object'}]]), 'calls2 registered once'
      );

      calls1.length = 0;
      calls2.clear();
      dispatcher.off('boo', handler1);
      dispatcher.fire('boo', {a: 'different object'});
      ...
      this.assertEqual(calls1.length, 0, 'calls1 should be empty');
      this.assertEqual(calls2.size, 0, 'calls2 should be empty');
    }
  ...
  /**
   * NexusHoratio libraries log issues here.
   *
  ...
  class Logger {
    ...
    #outro = (group, ...rest) => {
      ...
      if (group !== lastGroup) {
        this.#mq.post(`${this.name}: Group mismatch!  Passed ` +
  ...
  class LoggerTestCase extends NH.xunit.TestCase {
    ...
    testExpectMismatchedGroup() {
      ...
      this.assertEqual(messages, [
        'LoggerTestCase.testExpectMismatchedGroup: Group mismatch!  ' +
          'Passed "two", expected to see "one"',
      ]);
    }
