...
window.NexusHoratio.spa = (function spa() {
  ...
  class Page {
    ...
    static Service = class extends NH.base.Service {
      ...
      set page(val) {
        if (this.#page) {
          throw new Exception('page was already set once');
        }
        this.#page = val;
      }
  ...
  class PageTestCase extends NH.xunit.TestCase {
    ...
    testServicePagePropertyCannotBeAssignedTwice() {
      ...
      const spa = Symbol(this.id);
      const page = new PageTestCase.ADefaultPage(spa);
      const service = new Page.Service();
   ...
   * specific values for {@link PageDetails.readySelector}.
   *
   * This class logs a lot and modifies every element encounters, slowing down
   * pages.  It is unlikely it should always be enabled.
   *
   ...
   *
   * @example
   * $$('[data-counter]:not(svg,image,figure)')
  ...
  class WatchPage extends Page {
    ...
    static #Activator = class extends Page.Service {
