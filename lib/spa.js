 ...
 *
 * @external SPA
 * @see {@link https://developer.mozilla.org/en-US/docs/Glossary/SPA SPA
 * (Single-Page Applications)}.
 */

/**
 * Support for {@link external:SPA Single-Page Applications}.
 *
 * Depends on:
 * - {@link NexusHoratio.xunit}
 * - {@link NexusHoratio.base}
 * - {@link NexusHoratio.web}
 * @version 15
 * @license [GPL-3.0-or-later]{@link https://www.gnu.org/licenses/gpl-3.0-standalone.html}
 * @namespace NexusHoratio.spa
 */
window.NexusHoratio.spa = (function spa() {
  ...
  /**
   * Base class for handling various views of a {@link external:SPA
   * Single-Page Applications}.
   *
   * Pages are created by subclassing with at least a custom constructor that
   * provides {@link NexusHoratio.spa.Page~PageDetails PageDetails} via
   * `super()`.  The classes are then registered with a {@link
   * NexusHoratio.spa.SPA SPA} instance where they are instantiated with a
   * single parameter of that same {@link NexusHoratio.spa.SPA SPA}.
   *
   * @memberof NexusHoratio.spa
   */
  class Page {
     ...
     * @memberof NexusHoratio.spa.Page~
     * @property {NexusHoratio.spa.SPA} spa - SPA instance that manages this
     * Page.
     ...
     * @property {number} [readySelectorTimeout=3000] - Time to wait, in
     * milliseconds, for the readySelector.
     */

    /**
     * @param {NexusHoratio.spa.PageDetails} details - Details about the page.
     */
    constructor(details = {}) {
      if (new.target === Page) {
      ...
      ({
        readySelector: this.#readySelector = Page.#defaults.READY_SELECTOR,
        readySelectorTimeout:
          this.#readySelectorTimeout = Page.#defaults.READY_SELECTOR_TIMEOUT,
    ...
    /**
     * A convenience class for integrating {@link NexusHoratio.spa.Page} with
     * {@link NexusHoratio.base.Service}.
     *
     * When used with {@link NexusHoratio.spa.Page#addService addService},
     * subclasses will have the {@link NexusHoratio.spa.Page.Service#page
     * page} property set.
     *
    ...
    static Service = class extends NH.base.Service {
      ...
      /**
       * @type {NexusHoratio.spa.Page}
       * @alias NexusHoratio.spa.Page.Service#page
       */
      get page() {
        return this.#page;
      }
    ...
    /** @type {NexusHoratio.spa.SPA} */
    get spa() {
      return this.#spa;
    }

    /**
     * Register a new {@link NexusHoratio.base.Service}.
     *
     * @param {function(): NexusHoratio.base.Service} Klass - A service class
     * to instantiate.  If this is a {@link NexusHoratio.spa.Page.Service
     * Page.Service} subclass, the {@link NexusHoratio.spa.Page.Service#page
     * page} property is also set.
     * @param {...object} rest - Arbitrary objects to pass to constructor.
     * @returns {NexusHoratio.base.Service} Instance of Klass.
     */
    ...
    static #eventTypes = [
      ...
      /** @event NexusHoratio.spa.Page#activate */
      'activate',

      /** @event NexusHoratio.spa.Page#deactivate */
      'deactivate',
    ];

    #dispatcher = new NH.base.Dispatcher(...Page.#eventTypes);
    #id
    #logger
    #name
    #pathnameRE
     ...
     * @returns {external:Element} The element matched by #readySelector.
     */
    #waitUntilReady = async () => {
      ...
      try {
        element = await NH.web.waitForSelector(
          this.#readySelector, this.#readySelectorTimeout
        );
      } catch (e) {
        NH.base.issues.post(
  ...
  class PageTestCase extends NH.xunit.TestCase {
    ...
    static AnOverridePage = class extends Page {
      ...
      /**
       * @alias AnOverridePage
       * @param {object} spa - Testing.
       * @param {object} method - Testing.
       * @ignore
       */
   ...
   * useful values for {@link NexusHoratio.spa.Page~PageDetails PageDetails}'s
   * `readySelector`.
   *
   ...
   * `data-counter` value to see what the last thing loaded was.  Having
   * timestamps turned on in the console helps identify when initial loading
   * has completed.
   *
   * @example
  ...
  class WatchPage extends Page {
    ...
    /**
     * @param {NexusHoratio.spa.SPA} spaInstance - SPA instance that manages
     * this Page.
     */
   ...
   * pass in while creating a {@link NexusHoratio.spa.SPA SPA} instance.
   *
   * Instances of {@link NexusHoratio.spa.Page Page} will have access to
   * maintain state and shared information.
   *
   * A dispatcher is exposed to allow {@link NexusHoratio.spa.SPA SPA} to send
   * events.
   * @memberof NexusHoratio.spa
   */
  class Details {
    ...
    static #eventTypes = [
      ...
      /** @event NexusHoratio.spa.SPA#activate */
      'activate',

      /** @event NexusHoratio.spa.SPA#activated */
      'activated',

      /** @event NexusHoratio.spa.SPA#initialize */
      'initialize',

      /** @event NexusHoratio.spa.SPA#initialized */
      'initialized',
    ];

    #dispatcher = new NH.base.Dispatcher(...Details.#eventTypes);
    #id
    #logger
    #spa
  ...
  /**
   * A userscript driver for working with a {@link external:SPA Single-Page
   * Application}.
   *
   ...
   * to different views.
   *
   * Generally, a single instance of this class is created, and all {@link
   * NexusHoratio.spa.Page Page} subclasses are registered with it.  As the
   * user navigates through the SPA, this class will react to the changes and
   * enable and disable view specific Pages as appropriate.
   *
  ...
  class SPA {
    ...
    /**
     * @param {NexusHoratio.spa.Details} details - Implementation specific
     * details.
     */
    constructor(details) {
     ...
     * @param {NexusHoratio.spa.Page} Klass - A {@link NexusHoratio.spa.Page}
     * class to instantiate.
     * @returns {NexusHoratio.spa.SPA} This instance, for chaining.
     */
    register(Klass) {
     ...
     * @memberof NexusHoratio.spa.SPA~
     * @property {Set<NexusHoratio.spa.Page>} active - Currently active pages.
     * @property {Set<NexusHoratio.spa.Page>} old - Pages being deactivated.
     * @property {Set<NexusHoratio.spa.Page>} new - Pages being activated.
     */
    ...
    #activate = (pathname) => {
      ...
      for (const page of pages.old) {
        page.dispatcher.fire('deactivate');
      }
      for (const page of pages.new) {
        page.dispatcher.fire('activate');
      }
