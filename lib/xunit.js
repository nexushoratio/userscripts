...
window.NexusHoratio.xunit = (function xunit() {
  ...
  class TestCase {
    ...
    equalMap = (first, second) => {
      ...
      for (const [key, val1] of first.entries()) {
        if (second.has(key)) {
          const val2 = second.get(key);
          const equalFunc = this.getEqualFunc(val1, val2);
          const result = equalFunc(val1, val2);
          if (!result.equal) {
            equal = false;
            differences.push(
              '',
              `Difference with key: ${this.repr(key)}`,
              `Value in first : ${this.repr(val1)}`,
              `Value in second: ${this.repr(val2)}`,
            );
          }
        } else {
          equal = false;
          missingSecond.push(
            '',
            `Key missing from second: ${this.repr(key)}`,
            `Value in first : ${this.repr(val1)}`,
          );
        }
      }
      for (const [key, val2] of second.entries()) {
        if (!first.has(key)) {
          equal = false;
          missingFirst.push(
            '',
            `Key missing from first : ${this.repr(key)}`,
            `Value in second: ${this.repr(val2)}`,
          );
        }
      }
    ...
    equalSet = (first, second) => {
      ...
      for (const val of first.values()) {
        if (!second.has(val)) {
          ...
          missingSecond.push(
            '',
            `Value missing from second: ${this.repr(val)}`,
          );
        }
      }
      for (const val of second.values()) {
        if (!first.has(val)) {
          equal = false;
          missingFirst.push(
            '',
            `Value missing from first : ${this.repr(val)}`,
          );
        }
      }
      const details = [
  ...
  class GetTypeTestCase extends TestCase {
    ...
    testRegularClasses() {
      ...
      this.assertEqual(getType(this), 'GetTypeTestCase');
  ...
  class TestCaseTestCase extends TestCase {
    ...
    testEqualObject() {
      ...
      this.assertEqual(this.getEqualFunc({}, {a: 1}), this.equalObject);
...
\\[1, 3\\]`;
    ...
    testEqualMap() {
      ...
      this.assertEqual(this.getEqualFunc(m1, m2), this.equalMap);
...
Value in first : 42`;
      ...
      // Bug?  equalMap() looks up keys using the .has() method.  And do the
      // key as an object below does not get matched.  Everything else in this
      // library would treat them as equivalent.  Can revisit if it is a real
      // issue.
      // TODO(#183): Definitely a bug.  We have the same problem with Set(),
      // and if we fix it there, we should fix it here.
...
Key missing from first : \\{\\}
Value in second: "y"

Key missing from second: 3
Value in first : "c"

Key missing from second: \\{\\}
Value in first : "x"`;
      this.assertRaisesRegExp(
        TestCase.Fail,
        RegExp(expected, 'u'),
...
Value in second: Map\\(\\[\\["a", 43\\]\\]\\)`;
    ...
    testEqualSet() {
      ...
      this.assertEqual(this.getEqualFunc(s1, s2), this.equalSet);
      ...
      // TODO(#183): This should be equal; same problem in equalMap(),
      // probably others.  It is because we use .has().
      s1 = new Set([1, new Set(['a', 42])]);
      s2 = new Set([1, new Set(['a', 42])]);
      this.assertNotEqual(s1, s2, 'nested');
