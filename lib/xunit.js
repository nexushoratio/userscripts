...
window.NexusHoratio.xunit = (function xunit() {
  ...
  const testing = {
    enabled: false,
    missingDescriptionsAreErrors: false,
    testCases: [],
  };
  ...
  class TestCase {
    ...
    #checkAssertionCounts = () => {
      // TODO(#215): Crank these down as they are cleaned up.
      const MIN_ASSERTS = 7;
      const MAX_MISSING = 5;
  ...
  class TestCaseTestCase extends TestCase {
    ...
    testDoInstanceCleanupsWithError() {
      ...
      this.assertEqual(result.tests.size, 1, 'tests collected');
      this.assertEqual(result.errors.length, 1, 'errors');
      this.assertEqual(result.errors[0].error, 'Error', 'error type');
    }
    ...
    testDoInstanceCleanupsWithSkip() {
      ...
      this.assertEqual(result.tests.size, 1, 'tests collected');
      this.assertEqual(result.errors.length, 1, 'errors');
      this.assertEqual(result.errors[0].error, 'TestCase.Skip', 'error type');
    }
    ...
    testDoInstanceCleanupsWithFail() {
      ...
      this.assertEqual(result.tests.size, 1, 'tests collected');
      this.assertEqual(result.errors.length, 1, 'errors');
      this.assertEqual(result.errors[0].error, 'TestCase.Fail', 'error type');
    }
...
diff:       |
   2: abc123`;
      this.assertRaisesRegExp(
        ...
        RegExp(expected, 'u'),
        () => {
          this.assertEqual('abc1234', 'abc123', 'inner assert');
        },
        'first longer',
      );
...
diff:   |
   2: abxd`;
      this.assertRaisesRegExp(
        ...
        RegExp(expected, 'u'),
        () => {
          this.assertEqual('abcd', 'abxd', 'inner assert');
        },
        'diff in middle'
      );
...
Value in second: "b"`;
      this.assertRaisesRegExp(
        ...
        RegExp(expected, 'u'),
        () => {
          this.assertEqual(m1, m2, 'inner assert');
        },
        'same key, different values'
      );
