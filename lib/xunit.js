 ...
 * [xUnit](https://en.wikipedia.org/wiki/XUnit) style test framework.
 * @version 63
 * @license [GPL-3.0-or-later]{@link https://www.gnu.org/licenses/gpl-3.0-standalone.html}
 * @namespace NexusHoratio.xunit
 */
window.NexusHoratio.xunit = (function xunit() {
   ...
   *
   * Currently supports a Python-like `repr()` method.  Better output for
   * built-in container types is the primary goal.
   *
   ...
   * to extend support.
   *
   * The {@link NexusHoratio.xunit.TypeTool#addReprFunc addReprFunc} method
   * can allow users to register their own.
   *
   * Migrating equality testing out of {@link NexusHoratio.xunit.TestCase
   * TestCase} is a future goal.
   *
   * It was originally designed for testing, but there can be use cases such
   * as human readable logging as well.
   *
   * @memberof NexusHoratio.xunit
   * @todo [(#344)](https://github.com/nexushoratio/userscripts/issues/344)
   * WIP
   */
  class TypeTool {
     ...
     *
     * This is NOT equivalent to the built-in `typeof` operator.  Hence,
     * results are explicitly NOT lower-cased in order to reduce chances of
     * conflicts.
     *
     * @method
    ...
    getType = (item) => {
      ...
      if (type === 'Function') {
        if (String(item)
          .startsWith('class ')) {
          type = 'class';
        } else if (this.#builtInClasses.includes(item.name)) {
          type = 'class';
        }
      }
      if (type === 'Object') {
    ...
    /**
     * @callback NexusHoratio.xunit.TypeTool~ReprFunc
     * @param {object} item - Anything.
     * @returns {string} String version of item.
     */
     ...
     * @returns {NexusHoratio.xunit.TypeTool~ReprFunc} Function for this item.
     */
    getReprFunc(item) {
     ...
     * @param {NexusHoratio.xunit.TypeTool~ReprFunc} func - Function for this
     * type.
     */
    ...
    /**
     * @method
     * @implements {NexusHoratio.xunit.TypeTool~ReprFunc}
     ...
     * @method
     * @implements {NexusHoratio.xunit.TypeTool~ReprFunc}
     * @param {object[]} array - Array of anything.
     * @returns {string} String version of item.
     */
    reprArray = (array) => {
     ...
     * @method
     * @implements {NexusHoratio.xunit.TypeTool~ReprFunc}
     * @param {Map<object,object>} map - Any Map.
  ...
  /**
   * See {@link NexusHoratio.base~runTests} for an example of how this might
   * be used.
   *
   * @const {object} - Default testing support.
   * @property {boolean} enabled - Whether tests should be executed.
   * @property {NexusHoratio.xunit.TestCase[]} testCases - Registered
   * TestCases.
   * @memberof NexusHoratio.xunit
   */
  const testing = {
  ...
  class TestResult {
    ...
    /**
     * @typedef {object} NexusHoratio.xunit~TestResult~MethodResult
     * @property {string} methodName - Name of the associated {@link
     * NexusHoratio.xunit.TestCase} method.
     * @property {string?} error - Name of caught exception.
     * @property {string} message - Message from the test or framework.
     */

    /**
     * Unexpected exceptions.
     *
     * @type {NexusHoratio.xunit~TestResult~MethodResult[]}
     */
    errors = [];
     ...
     * All test executions.
     *
     * @type {Map<string,NexusHoratio.xunit~TestExecution>}
     */
    tests = new Map();
     ...
     *
     * @param {string} methodName - Name of the associated {@link
     * NexusHoratio.xunit.TestCase} method.
    ...
    addSuccess(methodName) {
      this.successes.push(methodName);
    }
    ...
    startTest(methodName) {
      ...
      this.tests.set(methodName, execution);
    }
     ...
     */
    stopTest(methodName) {
      this.tests.get(methodName).stop = Date.now();
    }
     ...
     *
     * @param {boolean} [formatted=false] - Try to line things up in columns.
   ...
   *
   * Many expected methods exist, such as {@link
   * NexusHoratio.xunit.TestCase#setUp setUp}, {@link
   * NexusHoratio.xunit.TestCase.setUpClass setUpClass}, {@link
   * NexusHoratio.xunit.TestCase#addCleanup addCleanup}, {@link
   * NexusHoratio.xunit.TestCase.addClassCleanup addClassCleanup}, etc.  No
   * `tearDown` methods, however; use {@link
   * NexusHoratio.xunit.TestCase#addCleanup addCleanup}.
   *
   ...
   * message.
   *
   * While the {@link NexusHoratio.xunit.TestCase#assertEqual assertEqual}
   * method will handle many cases by looking up special functions comparing
   * by type.  There may be times when what it can handle needs to be
   * enhanced.  There are currently two ways to make such enhancements.
   *
   * First, the method {@link NexusHoratio.xunit.TestCase#addEqualFunc
   * addEqualFunc} will allow the test method to register an additional
   * function for comparing two identical instances.
   *
   * Second, the property {@link NexusHoratio.xunit.TestCase#defaultEqual
   * defaultEqual} points to whatever *equalX()* function should be used if
   * one cannot be found, or if instances differ by type.  This fallback
   * defaults to {@link NexusHoratio.xunit.TestCase#equalEqEqEq equalEqEqEq}
   * which uses the strict equality (`===`) operator.  This can be explicitly
   * set in the test method.  The method {@link
   * NexusHoratio.xunit.TestCase#equalValueOf equalValueOf} will use the
   * instance's {@link
   * https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/valueOf
   * valueOf} method to get comparable values, and may be useful in such
   * cases.
   *
   ...
   * the default *equalX()* methods explicitly IGNORE order.
   *
   * Implementations for built-in types will be added as needed.
   *
   * All *assertX()* and *equalX()* methods should use {@link
   * NexusHoratio.xunit.TestCase#typeTool typeTool} to turn values into
   * strings for these error messages.
   *
   * TestCases should run only one test method per instance.  The name of the
   * method is registered during instantiation and invoked by calling the
   * {@link NexusHoratio.xunit.TestCase#run run} method.  Generally, a system,
   * like {@link NexusHoratio.xunit~TestRunner TestRunner} is used to register
   * a number of TestCases, discover the test methods, and invoke all of them
   * in turn.
   *
   * The sibling library, {@link NexusHoratio.base}, enhances {@link
   * NexusHoratio.xunit.testing} with a convenience function to run all
   * TestCases present in {@link NexusHoratio.xunit.testing.testCases
   * testCases}.  Similarly, this class provides a static method to register
   * such classes.
   *
   * @memberof NexusHoratio.xunit
   * @example
  ...
  class TestCase {
    ...
    constructor(methodName) {
      ...
      this.addEqualFunc('RegExp', this.equalRegExp);
     ...
     * @class
     * @extends Error
     * @alias NexusHoratio.xunit.TestCase~Error
     */
    static Error = class extends Error {

      /** @type {string} */
      get name() {
        return `TestCase~${this.constructor.name}`;
      }
    ...
    /**
     * @class
     * @extends NexusHoratio.xunit.TestCase~Error
     * @alias NexusHoratio.xunit.TestCase~Fail
     */
     ...
     * @class
     * @extends NexusHoratio.xunit.TestCase~Error
     * @alias NexusHoratio.xunit.TestCase~Skip
     */
     ...
     * @param {...object} rest - Arbitrary arguments to func.
     */
     ...
     *
     * @type {NexusHoratio.xunit.TestCase~EqualFunc}
     */
    defaultEqual
    ...
    /** @type {NexusHoratio.xunit.TypeTool} */
    get typeTool() {
     ...
     *
     * @param {NexusHoratio.xunit~TestResult} [result] - Instance for
     * accumulating results.  Typically, a test runner will pass in one of
     * these to gather results across multiple tests.
     * @returns {NexusHoratio.xunit~TestResult} Accumulated results (one is
     ...
     * @throws {NexusHoratio.xunit.TestCase~Skip}
     */
    skip(msg = '') {
     ...
     * @throws {NexusHoratio.xunit.TestCase~Fail}
     */
    fail(msg = '') {
     ...
     *
     * @param {object} arg - Argument to test.
     * @param {string} [msg=''] - Text to complement the failure message.
     */
    assertTrue(arg, msg = '') {
      this.#countAsserts(msg);
     ...
     *
     * @param {function(): Error} exc - Expected Error class.
     * @param {object} cause - Cause to match.
     * @param {function} func - Function to call.
     * @param {string} [msg=''] - Text to complement the failure message.
     */
    assertRaisesCause(exc, cause, func, msg = '') {  // eslint-disable-line max-params
      this.#countAsserts(msg);
      ...
      try {
        ...
        if (e instanceof exc) {
          const equalFunc = this.getEqualFunc(cause, e.cause);
          const result = equalFunc(cause, e.cause);
          if (result.equal) {
            return;
          }
          if (!result.details) {
            const s1 = this.repr(cause);
            const s2 = this.repr(e.cause);
            result.details = `\n${s1} !== ${s2}`;
          }
          failMsg = `Cause did not match:${result.details}`;
        } else {
     ...
     * NexusHoratio.xunit.TestCase#typeTool.repr}.
     *
     * @param {object} item - Anything.
     * @returns {string} String version of item.
     */
    repr(item) {
      return this.typeTool.repr(item);
    }

    /**
     * @typedef {object} NexusHoratio.xunit.TestCase~EqualOutput
    ...
    /**
     * @callback NexusHoratio.xunit.TestCase~EqualFunc
     * @param {object} first - First argument.
     * @param {object} second - Second argument.
     * @returns {NexusHoratio.xunit.TestCase~EqualOutput} Results of testing
     * equality.
     */
     ...
     * @param {object} first - First argument.
     * @param {object} second - Second argument.
     * @returns {NexusHoratio.xunit.TestCase~EqualFunc} Function that should
     * be used to test equality.
     */
    getEqualFunc(first, second) {
      let equal = this.defaultEqual;
      const t1 = this.typeTool.getType(first);
      const t2 = this.typeTool.getType(second);
      if (t1 === t2) {
     ...
     * @param {NexusHoratio.xunit.TestCase~EqualFunc} func - Function to call
     * to compare that type.
     */
    ...
    /**
     * @method
     * @implements {NexusHoratio.xunit.TestCase~EqualFunc}
     * @param {object} first - First argument.
     * @param {object} second - Second argument.
     * @returns {NexusHoratio.xunit.TestCase~EqualOutput} Results of testing
     * equality.
     ...
     * @method
     * @implements {NexusHoratio.xunit.TestCase~EqualFunc}
     * @param {Map<object,object>} first - First argument.
     * @param {Map<object,object>} second - Second argument.
     * @returns {NexusHoratio.xunit.TestCase~EqualOutput} Results of testing
     * equality.
     */
     ...
     * @method
     * @implements {NexusHoratio.xunit.TestCase~EqualFunc}
     * @param {Set<object>} first - First argument.
     * @param {Set<object>} second - Second argument.
     * @returns {NexusHoratio.xunit.TestCase~EqualOutput} Results of testing
     * equality.
     */
     ...
     * equality.
     */
    equalRegExp = (first, second) => {
      const equal = first.toString() === second.toString();
     ...
     *
     * @method
     * @param {Map<object,object>|Set<object>} container - Or any type with
     * similar signatures.
     * @returns {Map<object,object>|Set<object>} Clone of container with all
     * keys and values transformed into a string.
     */
  ...
  class TypeToolTestCase extends TestCase {

    testPrimitives() {
      const tt = new TypeTool();
      this.assertEqual(tt.getType(0), 'Number', 'zero');
      this.assertEqual(tt.getType(NaN), 'Number', 'Nan');
      this.assertEqual(tt.getType('0'), 'String', '"0"');
      this.assertEqual(tt.getType(true), 'Boolean', 'true');
      this.assertEqual(tt.getType(false), 'Boolean', 'false');
      this.assertEqual(tt.getType(456n), 'BigInt', '456n');
      this.assertEqual(tt.getType(/abc123/u), 'RegExp', '/regexp/');
      this.assertEqual(tt.getType(undefined), 'Undefined', 'undefined');
      this.assertEqual(tt.getType(null), 'Null', 'null');
    }
    ...
    testBuiltInFunctionLike() {
      const tt = new TypeTool();
      this.assertEqual(tt.getType(String('xyzzy')), 'String', 'string-xyzzy');
      this.assertEqual(tt.getType(new String('abc')), 'String', 'string-abc');
      this.assertEqual(tt.getType(String), 'Function', 'bare String');
      this.assertEqual(tt.getType(Symbol('xyzzy')), 'Symbol', 'symbol');
      this.assertEqual(tt.getType(Symbol), 'Function', 'bare Symbol');
      this.assertEqual(tt.getType(BigInt('123')), 'BigInt', 'BigInt()');
      this.assertEqual(tt.getType(BigInt), 'Function', 'bare BigInt');
      this.assertEqual(tt.getType(RegExp()), 'RegExp', 'RegExp()');
      this.assertEqual(tt.getType(RegExp('wxyz')), 'RegExp', 'RegExp()');
      this.assertEqual(
        tt.getType(new RegExp('xyz', 'u')), 'RegExp', 'RegExp()'
      );
      this.assertEqual(tt.getType(new Date()), 'Date', 'new Date');
      this.assertEqual(tt.getType(Date()), 'String', 'Date()');
      this.assertEqual(tt.getType(Date), 'class', 'bare Date');
      this.assertEqual(tt.getType(Math.min), 'Function', 'math.min');
      this.assertEqual(tt.getType(Math), 'Math', 'Math');
    }

    testBuiltinClasses() {
      const tt = new TypeTool();
      this.assertEqual(tt.getType({}), 'Object', '{}');
      this.assertEqual(tt.getType([]), 'Array', '[]');
      this.assertEqual(tt.getType(new Array()), 'Array', 'new array');
      this.assertEqual(tt.getType(Array), 'class', 'bare Array');
      this.assertEqual(tt.getType(new Map()), 'Map', 'map');
      this.assertEqual(tt.getType(Map), 'class', 'bare Map');
      this.assertEqual(tt.getType(new Set()), 'Set', 'set');
      this.assertEqual(tt.getType(Set), 'class', 'bare Set');
      this.assertEqual(tt.getType(new Error()), 'Error', 'error');
      this.assertEqual(tt.getType(Error), 'class', 'bare Error');
    }

    testRegularClasses() {
      const tt = new TypeTool();
      this.assertEqual(tt.getType(TestCase), 'class', 'bare TestCase');
      this.assertEqual(tt.getType(this), 'TypeToolTestCase', 'this');
      this.assertEqual(tt.getType(tt.getType), 'Function', 'bare tt.getType');
      this.assertEqual(tt.getType(TestCase.Skip), 'class', 'nested class');
    }

    testGetReprFunc() {
      const tt = new TypeTool();
      this.assertEqual(tt.getReprFunc(null), String, 'null');
      this.assertEqual(tt.getReprFunc(undefined), String, 'undefined');
      this.assertEqual(tt.getReprFunc(1), String, 'number');
      this.assertEqual(tt.getReprFunc(''), tt.reprString, 'string');
      this.assertEqual(tt.getReprFunc([]), tt.reprArray, 'array');
      this.assertEqual(tt.getReprFunc({}), tt.reprObject, 'object');
      this.assertEqual(tt.getReprFunc(new Map()), tt.reprMap, 'map');
      this.assertEqual(tt.getReprFunc(new Set()), tt.reprSet, 'set');
    }
    ...
    testAddReprFunc() {
      ...
      this.assertNotEqual(tt.getReprFunc(c), reprC, 'no reprC');

      // Act
      tt.addReprFunc('C', reprC);

      // Assert
      this.assertEqual(tt.getReprFunc(c), reprC, 'found reprC');
      this.assertEqual(tt.getReprFunc(null), String, 'null');
      this.assertEqual(tt.getReprFunc(undefined), String, 'undefined');
      this.assertEqual(tt.getReprFunc(1), String, 'number');
      this.assertEqual(tt.getReprFunc(''), tt.reprString, 'string');
      this.assertEqual(
        tt.getReprFunc(new String('str')), tt.reprString, 'new string'
      );
      this.assertEqual(tt.getReprFunc([]), tt.reprArray, 'array');
      this.assertEqual(
        tt.getReprFunc(new Array(1, 2, 3)), tt.reprArray, 'new array'
      );
      this.assertEqual(tt.getReprFunc({}), tt.reprObject, 'object');
      this.assertEqual(tt.getReprFunc(new Map()), tt.reprMap, 'map');
      this.assertEqual(tt.getReprFunc(new Set()), tt.reprSet, 'set');
    ...
    testReprPrimitives() {
      const tt = new TypeTool();
      this.assertEqual(tt.repr(1), '1', 'number');
      this.assertEqual(tt.repr(null), 'null', 'null');
      this.assertEqual(tt.repr(undefined), 'undefined', 'undefined');
      this.assertEqual(
        tt.repr(Symbol('qwerty')),
        'Symbol(qwerty)',
        'symbol'
      );
    }
    ...
    testReprString() {
      const tt = new TypeTool();
      this.assertEqual(tt.repr('a. b'), '"a. b"', 'string');
      this.assertEqual(tt.repr(new String('xyz')), '"xyz"', 'new string');
    }

    testReprArray() {
      const tt = new TypeTool();
      this.assertEqual(tt.repr(['b', 2]), '["b", 2]', 'mixed array');
      this.assertEqual(
        tt.repr(['b', [1, '2']]),
        '["b", [1, "2"]]',
        'nested array'
    ...
    testReprObject() {
      const tt = new TypeTool();
      this.assertEqual(tt.repr({a: '1'}), '{"a": "1"}', 'simple');
      this.assertEqual(
        tt.repr({b: {c: 'd', e: 1}}),
        '{"b": {"c": "d", "e": 1}}',
        'nested'
    ...
    testReprMap() {
      const tt = new TypeTool();
      this.assertEqual(tt.repr(new Map()), 'Map([])', 'empty');
      this.assertEqual(
        tt.repr(new Map([])),
        'Map([])',
        'empty init'
      ...
      this.assertEqual(
        tt.repr(new Map([[1, 'one'], ['two', 2]])),
        'Map([[1, "one"], ["two", 2]])',
        'with items'
      );
      this.assertEqual(
        tt.repr(new Map([[1, 'one'], ['map', new Map([['x', 3]])]])),
    ...
    testReprSet() {
      const tt = new TypeTool();
      this.assertEqual(tt.repr(new Set()), 'Set([])', 'empty');
      this.assertEqual(
        tt.repr(new Set([])),
        'Set([])',
        'empty init'
      ...
      this.assertEqual(
        tt.repr(new Set([1, 'b', 'b', 'xyz', 99])),
        'Set([1, "b", "xyz", 99])',
        'with items'
      );
      this.assertEqual(
        tt.repr(new Set([1, new Set(['x', 3]), 'qqq'])),
        'Set([1, Set(["x", 3]), "qqq"])',
        'nested'
      );
    }
  ...
  class TestCaseTestCase extends TestCase {
    ...
    testAddEqualFunc() {
      ...
      // Act
      this.addEqualFunc(this.typeTool.getType(c), equalC);
    ...
    testAssertEqualPrimitives() {
      ...
      this.assertEqual(/abc/u, /abc/u, '/regexp/');
      ...
      const baz1 = Symbol.for('baz');
      const baz2 = Symbol.for('baz');
      const baz3 = Symbol.for('baz3');
      this.assertEqual(baz1, baz2, 'same Symbol.for');
      this.assertNotEqual(baz1, baz3, 'different Symbol.for');
...
Value missing from second: 1`;
    ...
    testEqualRegExp() {
      let re1 = /abc123/u;
      let re2 = RegExp('abc123', 'u');
      let expected = '';

      this.assertEqual(this.getEqualFunc(re1, re2),
        this.equalRegExp,
        'equalFunc');
      this.assertEqual(re1, re2, 'equivalent');

      re1 = RegExp(undefined);
      re2 = new RegExp();
      this.assertEqual(re1, re2, 'func vs ctor');

      re1 = /xyz/um;
      re2 = /xyz/mu;
      this.assertEqual(re1, re2, 'rearranged flags');

      re1 = /abc123/u;
      re2 = /123abc/u;
      this.assertNotEqual(re1, re2, 'different sources');

      re1 = /xyz/u;
      re2 = /xyz/;
      this.assertNotEqual(re1, re2, 'different flags');

      re1 = /wxyz/ug;
      re2 = /xyz/i;
      expected = '/wxyz/gu !== /xyz/i';
      this.assertRaisesRegExp(
        TestCase.Fail,
        RegExp(expected, 'u'),
        () => {
          this.assertEqual(re1, re2);
        },
        'bit of everything'
      );
    }
    ...
    testAssertRaisesCause() {
      ...
      this.assertRaisesCause(
        ...
        'cause',
        () => {
          throw new Error('with a message', {cause: 'cause'});
        },
        'cause as string',
      );
      ...
      this.assertRaisesCause(
        ...
        {code: 1},
        () => {
          throw new Error('with a message', {cause: {code: 1}});
        },
        'cause as simple object',
      );

      this.assertRaisesRegExp(
        TestCase.Fail,
        /Expected Error, caught nothing :/u,
        () => {
          this.assertRaisesCause(
            Error,
            'cause',
      ...
      this.assertRaisesRegExp(
        TestCase.Fail,
        /Expected TypeError, caught Error :/u,
        () => {
          this.assertRaisesCause(
            TypeError,
            undefined,
      ...
      this.assertRaisesRegExp(
        ...
        () => {
          this.assertRaisesCause(
            ...
            'cause',
            () => {
              throw new Error('ignored', {cause: 'not-cause'});
            },
            'assert under test'
          );
        },
        'wrong cause, string v string'
      );
      ...
      this.assertRaisesRegExp(
        ...
        () => {
          this.assertRaisesCause(
            ...
            'cause',
            () => {
              throw new Error('ignored', {cause: {code: 0}});
            },
            'assert under test'
          );
        },
        'wrong cause, string v object'
      );
...
\\{"code": 0\\} !== "cause"`;
      this.assertRaisesRegExp(
        ...
        () => {
          this.assertRaisesCause(
            ...
            {code: 0},
            () => {
              throw new Error('ignored', {cause: 'cause'});
            },
            'assert under test'
          );
        },
        'wrong cause, object v string'
      );
  ...
  /**
   * Assembles and drives execution of {@link NexusHoratio.xunit.TestCase}s.
   * @memberof NexusHoratio.xunit~
   */
  class TestRunner {

    /**
     * @param {function(): NexusHoratio.xunit.TestCase} tests - TestCases to
     * execute.
     */
    constructor(tests) {
      const badKlasses = [];
     ...
     *
     * @returns {NexusHoratio.xunit~TestResult} Collected results.
     */
    runTests() {
      const result = new TestResult();
    ...
    /**
     * @method
     * @param {function(): NexusHoratio.xunit.TestCase} klass - TestCase to
     * process.
     */
     ...
     * @param {function(): NexusHoratio.xunit.TestCase} klass - TestCase to
     * process.
     * @param {NexusHoratio.xunit~TestResult} result - Result to use if any
     * errors.
     */
     ...
     * @param {NexusHoratio.xunit~TestResult} result - Result of the
     * execution.
     */
   ...
   *
   * @memberof NexusHoratio.xunit
   * @returns {NexusHoratio.xunit~TestResult} Accumulated results of these
   * tests.
   */
  function runTests() {
