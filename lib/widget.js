 ...
 * @see {@link https://developer.mozilla.org/en-US/docs/Web/API/HTMLStyleElement HTMLStyleElement}
 */
...
window.NexusHoratio.widget = (function widget() {
  ...
  class Widget {
     ...
     * @returns {external:HTMLStyleElement} Resulting style element.
     */
  ...
  /**
   * Tab item for the {@link NexusHoratio.widget.Tabs} widget.
   *
   * A TabItem manages two {@link NexusHoratio.widget.Widget Widget}s: tab and
   * panel.
   *
   * @ignore
   * @memberof NexusHoratio.widget
  ...
  class TabItem {
     ...
     * Set the tab.
     *
     * @param {Content} [tab=''] - Content to use for the tab.
     * @returns {TabItem} This instance for chaining.
     */
    setTab(tab = '') {
      this.#tab?.destroy();
      this.#tab = contentWrapper('tab content', tab);
     ...
     * Set the panel.
     *
     * @param {Content} [panel=''] - Content to use for the panel.
     * @returns {TabItem} This instance for chaining.
     */
    setPanel(panel = '') {
      this.#panel?.destroy();
  ...
  /* eslint-disable require-jsdoc */
  class TabItemTestCase extends NH.xunit.TestCase {
    ...
    testTab() {
      ...
      // Assert
      this.assertTrue(tabItem.tab instanceof Widget, 'default widget');
      this.assertEqual(
        tabItem.tab.name, 'StringAdapter tab content', 'default name'
      );

      // Act II - Custom
      tabItem.setTab(contentWrapper('custom content', 'new tab content'));

      // Assert
      this.assertEqual(
        tabItem.tab.name, 'StringAdapter custom content', 'custom content'
      );
      ...
      // Assert
      this.assertEqual(
        tabItem.tab.name, 'StringAdapter tab content', 'default again'
      );
    }
    ...
    testPanel() {
      ...
      // Assert
      this.assertTrue(tabItem.panel instanceof Widget, 'default widget');
      this.assertEqual(
        tabItem.panel.name, 'StringAdapter panel content', 'default name'
      );

      // Act II - Custom
      tabItem.setPanel(contentWrapper('custom content', 'new panel content'));

      // Assert
      this.assertEqual(
        tabItem.panel.name, 'StringAdapter custom content', 'custom content'
      );
      ...
      // Assert
      this.assertEqual(
        tabItem.panel.name, 'StringAdapter panel content', 'default again'
      );
    }
  ...
  class Tabs extends Widget {
    ...
    constructor(instanceName) {
      ...
      this.installStyle('nh-tabs', [
        'tabs[role="tabs"] {display: block;}',
        'tablist[role="tablist"] {display: block;}',
      ]);
      this.on('build', this.#onBuild)
        .on('destroy', this.#onDestroy);
    }

    /** @type {module.widget.Tab[]} */
    get tabItems() {
    ...
    #populateTabItems = () => {
      const me = this.#populateTabItems.name;
      this.logger.entered(me);

      for (const tabItem of this.#tabItems) {
        const tab = tabItem.tab;
        const panel = tabItem.panel;

        this.#tablist.append(tab.build().container);
        this.container.append(panel.build().container);
      }
  ...
  class TabsTestCase extends NH.xunit.TestCase {
    ...
    testTabItems() {
      ...
      const expected = [
        `^<tabs id="Tabs-[^-]*-${GUID}[^"]*">`,
        '<tablist role="tablist">',
        `<content id="StringAdapter-tab-content-${GUID}[^"]*"></content>`,
        '</tablist>',
        `<content id="StringAdapter-panel-content-${GUID}[^"]*"></content>`,
        '</tabs>$',
      ].join('');
      this.assertRegExp(w.container.outerHTML, RegExp(expected, 'u'));
    }
