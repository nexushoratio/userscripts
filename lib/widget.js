...
window.NexusHoratio.widget = (function widget() {
  ...
  class GridColumn {
    ...
    #validateInstance = () => {
      ...
      if (this.#renderFunc === GridColumn.defaultRenderFunc && !this.#field) {
        throw new WidgetError(
          'Needs at least one of renderFunc OR field'
        );
      }
  ...
  class GridColumnTestCase extends NH.xunit.TestCase {
    ...
    testEmptyConfig() {
      this.assertRaisesRegExp(
        WidgetError,
        /Needs at least one of renderFunc OR field/u,
        () => {
          new GridColumn({});
        }
      );
    ...
    testBadRenderFunc() {
      this.assertRaisesRegExp(
        ...
        () => {
          new GridColumn({renderFunc: null});
        }
      );
    }
    ...
    testNoTitleOrField() {
      // Assemble
      const col = new GridColumn({renderFunc: () => {}});

      // Assert
      this.assertEqual(col.title, '');
    }
    ...
    testCustomRenderer() {
      ...
      function renderFunc(record, field) {
        const fld = field ?? 'no-field';
        return contentWrapper(this.id, `${record.name}|${record.job}|${fld}`);
      }

      const col = new GridColumn({renderFunc: renderFunc});
      const record = {name: 'Bob', job: 'Artist'};
    ...
    #populateBuilt = () => {
      for (const row of this.#data) {
        ...
        for (const col of this.#columns) {
          built.push(contentWrapper(col.field, col.render(row)));
        }
  ...
  class GridTestCase extends NH.xunit.TestCase {
    ...
    testBuildWithData() {
      ...
      w.columns.push(
        ...
        new GridColumn({title: 'Type', renderFunc: renderType}),
      );
      ...
      const expected = [
        ...
        '<td><content id="StringAdapter-undefined-.*-container">',
        'juvenile, human',
        '</content></td>',
