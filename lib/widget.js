...
window.NexusHoratio.widget = (function widget() {
  ...
  class Layout extends Widget {
    ...
    /** @type {Widget} */
    get bottom() {
      return this.#panel.get(Layout.BOTTOM);
    }

    /** @type {Widget} */
    get left() {
      return this.#panel.get(Layout.LEFT);
    }

    /** @type {Widget} */
    get main() {
      return this.#panel.get(Layout.MAIN);
    }

    /** @type {Widget} */
    get right() {
      return this.#panel.get(Layout.RIGHT);
    }

    /** @type {Widget} */
    get top() {
      return this.#panel.get(Layout.TOP);
    }

    /**
     * Sets a panel for this instance.
     *
     * @param {string} panel - Panel to set (recommend to use {Layout.MAIN}
     * and friends.
    ...
    #onBuild = (...rest) => {
      ...
      const top = this.top?.container ?? document.createElement('div');
      const left = this.left?.container ?? document.createElement('div');
      const main = this.main?.container ?? document.createElement('div');
      const right = this.right?.container ?? document.createElement('div');
      const bottom = this.bottom?.container ?? document.createElement('div');
      const middle = document.createElement('div');
      middle.append(left, main, right);
      this.container.replaceChildren(top, middle, bottom);
  ...
  class LayoutTestCase extends NH.xunit.TestCase {
    ...
    testPanelsStartEmpty() {
      // Assemble
      const w = new Layout(this.id);

      // Assert
      this.assertEqual(w.main, undefined, 'main');
      this.assertEqual(w.top, undefined, 'top');
      this.assertEqual(w.bottom, undefined, 'bottom');
      this.assertEqual(w.left, undefined, 'left');
      this.assertEqual(w.right, undefined, 'right');
    }
    ...
    testEmptyBuilds() {
      ...
      this.assertEqual(w.container.innerHTML, expected);
    }
